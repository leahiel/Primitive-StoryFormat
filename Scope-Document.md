// Format guide to use GitHub's version of markdown. example https://github.com/inkle/ink/blob/master/Documentation/WritingWithInk.md
// TODO: Write more about the technical aspects required, such as the parser, tag management system and whatnot. Also about looping passages in Version 2.0.
// TODO: I need a term for the screen with the export options.

Scope Document for Primitive, a Twine-to-EPUB IF StoryFormat.
...
Please note that Version 0.1 will be incredibly similar to WritingFantasy, a StoryFormat made by Souphie Houlden. <https://sophiehoulden.com/twine/writingfantasy_guide.html> Primitive aims to surpass WritingFantasy in functionality starting in version 0.2.
...
2h) Passages should be able to be Hypertexted linked to. i.e. http://localhost:8080/#31
RESEARCH: In fact, this may be a reasonable EPUB requirement for the Table of Contents. If it is not, move 2h to Version 0.3.
2i) All links should have that "(turn to n)" added to them.
...
RESEARCH: Should "Chapters", as a whole, be added? Having a link for each Passage in the Table of Contents may be extreme. Consider "Our Cabin Was Cold", which has ~88 "Generated" Passages, but only 8 Chapters.
...
NOTE: The various forms of comments are for viability with different software that authors commonly use.
...
11) Separate CSS files compatibility should be accepted for each exported version that Primitive Offers.
...
2) A word and passage counter that does not consider the additional generated passages should be available.
2a) Erm, and also one that does consider the additional generated passages.
...
TECHNICAL: Adding and subtracting Integer State Variables, and using them in If Statements wouldn't be hard. The only notable thing is that I would have to have the Author give us the possible maximum and minimum values for each Integer State Variable. However, I'm not sure how the technical portion of adding Integer State Variables would work. i.e. How would I keep track of them in the Generated Passage Names?
Perhaps I could do something like `::P-ISV1-ISV2-ISV3-NBSVs`.
...
# Notes from my GameBook playthroughs
GameBooks Played: 
